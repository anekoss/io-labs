...
static long process(const char __user
...
static ssize_t device_write(struct file *filep, const char __user
...
r->
result = process(buffer, len, offset);
r->
...
static ssize_t device_read(struct file *file, char __user
...
struct list_res, list);
switch (entry->error) {
case NO_ERROR: {
snprintf(buf
+(i*LONG_STR_LEN), LONG_STR_LEN, "Result %ld: %ld\n", list_size - i, entry->result);
printk(KERN_NOTICE
...
printk(KERN_NOTICE
"%s: Sum of all correct expressions: %lld\n", THIS_MODULE->name, all_sum);
...
init(void) {
    ...
    if (IS_ERR(class)) {
        ...
        printk(KERN_ALERT
        "%s: Failed to register device class: %s\n", THIS_MODULE->name, CLASS_NAME);
        return PTR_ERR(class);
    } else {
        printk(KERN_INFO
        "%s: Registered device class: %s\n", THIS_MODULE->name, CLASS_NAME);
    }
